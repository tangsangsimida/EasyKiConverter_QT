...
def set_logger(log_file: str, log_level: int) -> None:
    """
    设置应用程序的日志系统，支持文件和终端输出
    Setup application logging system with file and terminal output support
    
    参数:
    Args:
        log_file (str): 日志文件路径，为空字符串时不写入文件 / Log file path, no file output if empty string
        log_level (int): 日志级别（如logging.INFO, logging.DEBUG等）/ Log level (e.g. logging.INFO, logging.DEBUG)
    """
...
def id_already_in_symbol_lib(
    ...
    """
    检查符号库中是否已存在指定名称的组件
    Check if a component with specified name already exists in the symbol library
    
    参数:
    Args:
        lib_path (str): 符号库文件路径 / Symbol library file path
        component_name (str): 组件名称 / Component name
        kicad_version (KicadVersion): KiCad版本枚举 / KiCad version enum
        
    返回:
    Returns:
        bool: 存在返回True，不存在返回False / True if exists, False if not
    """
...
def get_local_config() -> dict:
    if not os.path.isfile("easyeda2kicad_config.json"):
        with open(file="easyeda2kicad_config.json", mode="w", encoding="utf-8") as conf:
            json.dump(
                {"updated_at": datetime.utcnow().timestamp(), "version": "0.0.1"},
                conf,
                indent=4,
                ensure_ascii=False,
            )
