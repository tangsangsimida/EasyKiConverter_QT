#!/usr/bin/env python3
"""
测试修复符号定义格式后的符号库导出
"""

import sys
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src'))

from core.kicad.parameters_kicad_symbol import KicadVersion, KiSymbol, KiSymbolInfo
from core.utils.symbol_lib_utils import add_component_in_symbol_lib_file

# 创建一个测试符号
test_symbol = KiSymbol(
    info=KiSymbolInfo(
        name="RP2040",
        prefix="U",
        package="",
        manufacturer="",
        datasheet="",
        lcsc_id="",
        jlc_id="",
    )
)

# 导出为KiCad v6格式
exported_symbol = test_symbol.export(KicadVersion.v6)
print("=== 导出的符号内容 ===")
print(exported_symbol)

# 创建基础符号库文件内容
lib_header = """(kicad_symbol_lib
  (version 20211014)
  (generator "https://github.com/tangsangsimida/EasyKiConverter")
)"""

# 创建测试文件
test_file_path = "/tmp/test_symbol_definition.kicad_sym"

# 写入基础库文件
with open(test_file_path, 'w', encoding='utf-8', newline='\n') as f:
    f.write(lib_header)

print("\n=== 基础符号库文件 ===")
with open(test_file_path, 'r', encoding='utf-8') as f:
    content = f.read()
    print(content)

# 添加组件到符号库
add_component_in_symbol_lib_file(test_file_path, exported_symbol, KicadVersion.v6)

# 读取结果
with open(test_file_path, 'r', encoding='utf-8') as f:
    result = f.read()

print("\n=== 添加组件后的符号库文件 ===")
print(result)

# 检查文件结构
lines = result.split('\n')
print(f"\n文件总行数: {len(lines)}")

# 查找符号定义
symbol_start = -1
symbol_end = -1
for i, line in enumerate(lines):
    if '(symbol "RP2040"' in line and symbol_start == -1:
        symbol_start = i
    if symbol_start != -1 and line.strip() == '  )' and i > symbol_start:
        symbol_end = i
        break

if symbol_start != -1 and symbol_end != -1:
    print(f"\n符号定义从第{symbol_start+1}行到第{symbol_end+1}行")
    print("符号定义内容:")
    for i in range(symbol_start, symbol_end+1):
        print(f"  {i+1}: {lines[i]}")
else:
    print("\n未找到完整的符号定义")